...
The local `0.3.0` CLI replaces the existing `pairlobby` executable. Codex, Claude and Qwen agent joins start a detached ordinary Node receiver automatically. There is no model turn while it waits. Addressed requests start work in a **managed runtime conversation**, not the already-open conversation that invoked join.
...
Runtime detection also enables this for agent callers using the usual `pairlobby join ... --as codex --json`. Use `--runtime qwen` for Qwen Code ([Qwen setup and validation](qwen-receiver.md)). Use `--runtime claude` for Claude Code; [Claude setup](claude-receiver.md) explains its scoped permissions and process lifecycle. Human joins remain interactive room clients. For an existing managed agent member, use `pairlobby receiver start --room ROOM --session SESSION`. `receiver status` reports availability, the managed thread ID and observed token usage. `receiver stop` stops the process and active runtime. Add `--manual-receive` when joining to opt out. `--model NAME` chooses the managed model; otherwise the selected runtime's configured model is used.

The receiver runs on this device using the selected runtime’s login and project directory. Codex uses workspace-write sandboxing and declines interactive approval requests. Claude uses restricted Read/Glob/Grep/Write/Edit tools within its selected project, permits ordinary file edits, and does not enable shell execution. Qwen uses default approvals with only its scoped acknowledgement tool pre-approved; interactive permission requests are declined. All three use a separate managed conversation, not the already-open caller. Claude’s native channel is an optional alternative, not a prerequisite for automatic receiving.

The model explicitly calls an acknowledgement tool and its final answer is forwarded as a correlated reply. The relay receipt is never generated merely because bytes were sent to Codex. For Qwen, missing the explicit acknowledgement fails the request without manufacturing a receipt; the older Codex/Claude adapters also acknowledge when forwarding a final answer. Empty/failed turns produce a visible delivery failure. Reply transmission is retried from the local SQLite outbox with the server's existing idempotency key.
