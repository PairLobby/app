...
test('channel requires explicit agent ACK and exact final replies over real MCP and HTTP', async () => {
    ...
    const asks = await Promise.all(
        ['first', 'second'].map((text) =>
            api.send(owner.roomId, owner.participantCredential, {
                type: 'message',
    ...
    const transport = new StdioClientTransport({
        ...
        args: [resolve('packages/cli/dist/main.js'), 'channel', '--room', owner.roomId, '--session', sessionId, '--allow-from', owner.participantId],
        env: {PAIRLOBBY_DATA_DIR: directory},
        stderr: 'pipe'
    });
    const mcp = new Client({name: 'test-receiver', version: '1.0.0'});
    const notices: unknown[] = [];
    mcp.setNotificationHandler(z.object({method: z.literal('notifications/claude/channel'), params: z.object({content: z.string(), meta: z.record(z.string())})}), (notice) => {
        notices.push(notice);
    });
    try {
