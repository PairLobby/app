...
export async function runChannel(store: LocalStore,roomRef: string|undefined,sessionRef: string|undefined,senders: string|undefined): Promise<number> {
    if(!senders) throw new UsageError('channel requires --allow-from with comma-separated participant IDs approved by the human');
    const allowed=new Set(senders.split(',').map(s=>s.trim()).filter(Boolean));
    const {room,session,credential,client}=select(store,roomRef,sessionRef);
    if(session.kind!=='agent'||session.role==='guest') throw new UsageError('channel requires an existing agent member session');
    const lock=join(store.directory,`channel-${session.sessionId}.lock`);
    let fd: number;
    try {fd=openSync(lock,'wx',0o600);}
    catch {
        let alive=true;
        try {const pid=Number(readFileSync(lock,'utf8'));process.kill(pid,0);} catch {alive=false;}
        if(alive) throw new UsageError('another channel already owns this participant; do not run two runtimes as one agent');
    ...
    const tool=(name:string,description:string,text=false)=>({name,description,inputSchema:{type:'object' as const,properties:{eventId:{type:'string'},...(text?{text:{type:'string'}}:{})},required:text?['eventId','text']:['eventId'],additionalProperties:false}});
    server.setRequestHandler(ListToolsRequestSchema,async()=>({tools:[
        ...
        {name:'list_pending_requests',description:'Recover every pending request addressed to this participant, including old messages.',inputSchema:{type:'object',properties:{},additionalProperties:false}},
    ]}));
    const content=(value:unknown)=>({content:[{type:'text' as const,text:JSON.stringify(value)}]});
    server.setRequestHandler(CallToolRequestSchema,async request=>{
        try {
            if(request.params.name==='list_pending_requests') {const page=await client.requests(room.roomId,credential,0,100,session.participantId);return content({...page,requests:page.requests.filter(r=>allowed.has(r.from))});}
            const input=request.params.arguments ?? {};
            if(typeof input.eventId!=='string') throw new Error('eventId is required');
            const target=await client.request(room.roomId,credential,input.eventId);
            if(target.to!==session.participantId || !allowed.has(target.from)) throw new Error('this request is not approved for this participant');
            ...
            if(target.receivedAt===null) throw new Error('Call acknowledge_message immediately before starting work; this request has no explicit acknowledgement yet');
            if(!['reply_to_message','progress_message'].includes(request.params.name)) throw new Error('unknown tool');
            if(typeof input.text!=='string'||!input.text.trim()) throw new Error('a non-empty response is required');
        ...
        } catch(error) {return {isError:true,content:[{type:'text' as const,text:error instanceof Error?error.message:'The room request failed'}]};}
    });
    const supervisor=new DeliverySupervisor({
        notify:async(request,reminder)=>{
            await server.notification({method:'notifications/claude/channel',params:{content:`${reminder?'REMINDER: ':''}${request.text}\n\nAcknowledge ${request.eventId} now, then provide a final reply using reply_to_message.`,meta:{room_id:room.roomId,event_id:request.eventId,sender_id:request.from}}});
        },
        fail:async(request,reason)=>{await client.deliveryFailed(room.roomId,credential,request.eventId,reason);process.stderr.write(`PairLobby delivery failure: ${request.eventId}: ${reason}\n`);},
    },allowed);
    let stopped=false;
    let finish!:()=>void;
    const done=new Promise<void>(resolve=>{finish=resolve;});
    const stop=()=>{stopped=true;client.closeLive();finish();};
    server.onclose=stop;
    server.oninitialized=()=>{void monitor().catch(error=>{process.stderr.write(`PairLobby channel stopped: ${error instanceof Error?error.message:'connection failure'}\n`);stop();});};
    async function monitor() {
        let cursor=0,nextCheck=0,paused=false;
            ...
            try {
                ...
                for(const event of events.events) {
                    ...
                    if(event.recipientId===session.participantId && (event.type==='control.pause'||event.type==='control.resume')) {
                        ...
                        await server.notification({method:'notifications/claude/channel',params:{content:`The human requested ${paused?'pause':'resume'} (revision ${event.payload.revision}). ${paused?'Stop accepting new work.':'You may resume work.'} Acknowledge the actual control outcome with pairlobby ack --room ${room.roomId} --session ${session.sessionId}.`,meta:{room_id:room.roomId,event_id:event.eventId}}});
                    }
                    if(event.type==='participant.revoked' && event.payload.participantId===session.participantId) {stop();return;}
                }
    ...
    finally {client.closeLive();await server.close();try {if(readFileSync(lock,'utf8')===String(process.pid)) unlinkSync(lock);}catch{}process.removeListener('SIGINT',stop);process.removeListener('SIGTERM',stop);}
}
