...
export class DeliverySupervisor {
    ...
    async tick(requests: MessageRequest[],now=Date.now()): Promise<number> {
        ...
        for(const id of this.attempts.keys()) if(!active.has(id)) this.attempts.delete(id);
        ...
        for(const request of requests) {
            if(request.responseEventId) continue;
            ...
            if(!attempt && request.receivedAt===null && inflight>=3) {next=Math.min(next,now+ACK_TIMEOUT_MS);continue;}
            if(!attempt) {attempt={count:0,next:now,progress:0,failed:false};this.attempts.set(request.eventId,attempt);if(request.receivedAt===null) inflight++;}
            const progress=Math.max(request.receivedAt ?? 0,request.progressAt ?? 0);
            if(progress>attempt.progress) {attempt.progress=progress;attempt.count=0;attempt.next=progress+REPLY_TIMEOUT_MS;}
            if(now<attempt.next) {next=Math.min(next,attempt.next);continue;}
            if(attempt.count>=3) {
                await this.target.fail(request,request.receivedAt===null?'No agent acknowledgement after three channel deliveries. Check whether the channel is enabled and the runtime is available.':'The agent acknowledged this request but did not provide a final reply or fresh progress after repeated reminders.');
                attempt.failed=true;if(request.receivedAt===null) inflight--;continue;
            }
            try {await this.target.notify(request,attempt.count>0);}
            catch { /* Transport success is not acknowledgement; retry the same ID. */ }
            attempt.count++;
