    ...
    const [value, unit] =
        magnitude < 60
            ...
            : magnitude < 3600
              ...
              : magnitude < 86_400 ? [Math.round(magnitude / 3600), 'hour'] : [Math.round(magnitude / 86_400), 'day'];
...
export function renderRooms(rooms: RoomEntry[], now = Date.now()): void {
    ...
    for (const room of sorted) {
        ...
        if (room.sessions.length === 0) {
            out('  no sessions on this device');
        }
        for (const session of room.sessions) {
            const runtime = session.runtime ? ` ${session.runtime}` : '';
            out(`  ${session.displayName}${runtime}  ${dim(session.sessionId)}`);
            out(
                `    ${session.kind}${session.role === 'controller' ? ', controller' : ''}  ·  read through #${session.lastReadSeq}  ·  joined ${relativeTime(session.joinedAt, now)}`
            );
            // What a human needs to leave the room and go talk to this agent directly.
            if (session.conversationId) {
                out(`    conversation ${session.conversationId}`);
            } else if (session.kind === 'agent') {
                out(`    ${dim('conversation unknown — pass --conversation or run: pairlobby session --conversation <id>')}`);
            }
            if (session.terminal) {
                out(`    ${dim(`terminal ${session.terminal}`)}`);
            }
            out(`    ${dim(session.cwd)}`);
        }
        out('');
...
export function renderWatchHeader(snapshot: RoomSnapshot, meParticipantId: string, mySessionId: string, now = Date.now()): void {
    ...
    if (me) {
        out(`you are ${me.displayName}  ${dim(mySessionId)}`);
    }
...
function names(participants: ParticipantLabel[]): string {
...
export function renderRoomList(entries: RoomListEntry[], now = Date.now()): void {
    ...
    for (const entry of entries) {
        ...
        if (snapshot) {
            ...
            const agents = active.filter((participant) => participant.kind === 'agent').length;
            out(
                `    ${active.length} in the room: ${people} ${people === 1 ? 'person' : 'people'}, ${agents} ${agents === 1 ? 'agent' : 'agents'}  ${dim(active.map((participant) => participant.displayName).join(', '))}`
            );
...
export function renderSnapshot(snapshot: RoomSnapshot, now = Date.now()): void {
    ...
    for (const participant of snapshot.participants) {
        const flags = [
            participant.revoked ? 'removed' : null,
            participant.left ? 'left' : null,
            participant.paused ? 'paused' : null,
            participant.role === 'controller' ? 'controller' : null
        ].filter(Boolean);
        ...
        out(`    ${capability}`);
        if (participant.controlRevision > 0) {
            out(`    control revision ${participant.controlRevision}: ${participant.acknowledgedOutcome ?? 'no acknowledgement yet'}`);
        }
    }
}
...
function describe(event: RoomEvent): string {
    switch (event.type) {
        ...
        case 'handover.offered':
            return `handover ${event.payload.handoverId} revision ${event.payload.revision}: ${event.payload.document.metadata.goal}`;
        case 'handover.accepted':
            return `accepted handover ${event.payload.handoverId} revision ${event.payload.revision}`;
