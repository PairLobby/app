...
export function resolveServer(options: {server?: string | undefined; local?: boolean | undefined}): string {
    ...
    if (options.server && options.local) throw new UsageError('use either --server or --local, not both');
    if (options.server) return options.server;
    if (options.local) return DEFAULT_LOCAL_SERVER;
...
export function resolveRoom(store: LocalStore, reference?: string): RoomEntry {
    ...
    if (!wanted) {
        ...
        if (rooms.length === 0) throw new UsageError('no rooms on this device yet; run "pairlobby create" or "pairlobby join <code>"');
    ...
    if ('room' in resolved) return resolved.room;
    if ('ambiguous' in resolved) throw new UsageError(`"${wanted}" matches several rooms: ${resolved.ambiguous.map((entry) => `${entry.name} (${entry.roomId})`).join(', ')}`);
...
export function resolveSession(room: RoomEntry, reference?: string): SessionEntry {
    ...
    if (wanted) {
        ...
        if (!match) throw new UsageError(`no session "${wanted}" in room ${room.name}`);
        return match;
    }
    if (room.sessions.length === 1) return room.sessions[0]!;
    if (room.sessions.length === 0) throw new UsageError(`this device has no session in room ${room.name}`);
...
export function select(store: LocalStore, roomRef?: string, sessionRef?: string): Selection {
    ...
    if (!credential) throw new UsageError(`no credential stored for session ${session.sessionId}; rejoin the room`);
...
export function controllerCredential(store: LocalStore, room: RoomEntry): string {
    ...
    if (!credential) throw new UsageError(`this device does not hold the controller credential for ${room.name}`);
    return credential;
}

/** Resolves a recipient by participant id or unambiguous display name. */
export function resolveRecipient(participants: {participantId: string; displayName: string; revoked: boolean; left: boolean}[], reference: string): string {
    const byId = participants.find((participant) => participant.participantId === reference);
    if (byId) return byId.participantId;
    ...
    if (matches.length === 1) return matches[0]!.participantId;
    if (matches.length > 1) throw new ProtocolError('invalid_request', `"${reference}" names ${matches.length} participants; use one of these ids: ${matches.map((participant) => participant.participantId).join(', ')}`);
