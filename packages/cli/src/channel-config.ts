...
export function configureClaude(store: LocalStore, roomRef: string | undefined, sessionRef: string | undefined, senders: string | undefined) {
    ...
    if (!senders || senders.split(',').some((id) => !/^pt_[0-9A-Z]{26}$/.test(id.trim()))) {
        throw new UsageError('--allow-from must list the participant IDs the human authorizes to message Claude');
    }
    ...
    const mcpPath = join(directory, 'mcp.json'),
        ...
        launchPath = join(directory, 'launch.sh');
    ...
    const argv = [
        'claude',
        ...(session.conversationId ? ['--resume', session.conversationId] : []),
        '--mcp-config',
        mcpPath,
        '--settings',
        settingsPath,
    ...
    return {
        ...
        command,
        activationRequired:
            "Exit the existing Claude session, run the launch script, and accept Claude's custom-channel and MCP consent prompts. Shell/tool permissions are not bypassed."
    };
}
