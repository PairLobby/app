...
import {reserveOnlineInvite} from './online';

type JoinAuthorization = {status: number; message: string};
...
export class Workspace extends DurableObject<Env> {
    ...
    constructor(ctx: DurableObjectState, env: Env) {
        ...
        this.service = new RoomService(this.store, () => Date.now(), {
            reserve: (code, roomId, expiresAt) => reserveOnlineInvite(this.env, code, roomId, expiresAt, this.current!.id, this.teamId)
        });
        ctx.storage.sql.exec(`
            CREATE TABLE IF NOT EXISTS hosted_meta(key TEXT PRIMARY KEY, value TEXT NOT NULL);
            CREATE TABLE IF NOT EXISTS room_privacy(room_id TEXT PRIMARY KEY,owner_id TEXT NOT NULL,private INTEGER NOT NULL);
            CREATE TABLE IF NOT EXISTS room_allowed(room_id TEXT NOT NULL,user_id TEXT NOT NULL,PRIMARY KEY(room_id,user_id));
            CREATE TABLE IF NOT EXISTS room_accounts(room_id TEXT NOT NULL,participant_id TEXT NOT NULL,user_id TEXT NOT NULL,PRIMARY KEY(room_id,participant_id));
        ...
        if (
            !ctx.storage.sql
                ...
                .toArray()
                .some((column) => column.name === 'replies')
        ) {
            ctx.storage.sql.exec('ALTER TABLE usage ADD COLUMN replies INTEGER NOT NULL DEFAULT 0');
        }
    ...
    async usage() {
        const usage = this.ctx.storage.sql.exec<{period: number; messages: number; requests: number}>('SELECT * FROM usage ORDER BY period DESC LIMIT 1').toArray()[0] ?? {
            period: 0,
            messages: 0,
            requests: 0
        };
    ...
    private extraBlocks(period: number) {
        return this.ctx.storage.sql.exec<{n: number}>('SELECT coalesce(sum(blocks),0) AS n FROM usage_grants WHERE period=?', period).one().n;
    }
    ...
    async startCheckout(workspaceId: string, user: Account, plan: Plan) {
        return this.serial(async () => {
            ...
            if (!workspace || workspace.owner_id !== user.id) {
                fail(403, 'Only the billing owner can subscribe');
            }
    ...
    async reconcileUsage(workspaceId: string, sessionId: string) {
        return this.serial(async () => {
            ...
            if (!workspace || customer !== workspace.customer_id || checkout.metadata?.workspaceId !== workspaceId || checkout.metadata.kind !== 'usage') {
                fail(403, 'Usage purchase does not match workspace');
            }
            const blocks = Number(checkout.metadata.blocks);
            const period = Number(checkout.metadata.period);
            if (
                ...
                checkout.currency !== this.env.CURRENCY ||
                checkout.amount_total !== blocks * 1000
            ) {
                fail(400, 'Invalid usage purchase');
            }
            ...
            this.ctx.storage.sql.exec(
                'INSERT INTO usage_grants(session_id,period,blocks) VALUES(?,?,?) ON CONFLICT(session_id) DO UPDATE SET blocks=excluded.blocks',
                checkout.id,
                period,
                granted
            );
    ...
    async reconcileBilling(workspaceId: string, subscriptionId: string, eventId: string) {
        return this.serial(async () => {
            if (this.ctx.storage.sql.exec('SELECT id FROM reconciled_events WHERE id=?', eventId).toArray().length) {
                return;
            }
            const workspace = await this.env.DB.prepare('SELECT * FROM workspaces WHERE id=?').bind(workspaceId).first<WorkspaceRecord>();
            if (!workspace) {
            ...
            if (workspace.customer_id !== customer || subscription.metadata.workspaceId !== workspaceId) {
                fail(403, 'Subscription does not match workspace');
            }
            if (workspace.subscription_id && workspace.subscription_id !== subscriptionId) {
                ...
                if (!['canceled', 'incomplete_expired'].includes(previous.status)) {
                    fail(409, 'Another subscription is already linked');
                }
            }
            const item = subscription.items.data[0];
            const plan = Object.entries(prices(this.env)).find(([, id]) => id && id === item?.price.id)?.[0] ?? null;
            const status =
                isPlan(plan) && subscription.items.data.length === 1 && item?.quantity === 1 && item.price.unit_amount === PLANS[plan].priceCents && item.price.currency === this.env.CURRENCY && item.price.recurring?.interval === 'month' && item.price.recurring.interval_count === 1 ? subscription.status : 'inactive';
            await this.env.DB.prepare('UPDATE workspaces SET plan=?,status=?,subscription_id=?,period_start=?,period_end=?,last_synced_at=? WHERE id=?')
                .bind(plan, status, subscription.id, (item?.current_period_start ?? 0) * 1000, (item?.current_period_end ?? 0) * 1000, Date.now(), workspaceId)
                .run();
            ...
            if (status !== 'active') {
                for (const ws of this.ctx.getWebSockets()) ws.close(1008, 'Subscription is inactive');
            }
        });
    }
        ...
        return this.serial(async () => {
            try {
                ...
                if (error instanceof ProtocolError) {
                    return json({error: {code: error.code, message: error.message, ...error.details}}, error.httpStatus);
                }
                if (error instanceof HttpError) {
                    return json({error: {code: error.status === 429 ? 'quota_exceeded' : 'invalid_request', message: error.message}}, error.status);
                }
                if (error instanceof Error && error.name === 'ZodError') {
                    return json({error: {code: 'invalid_request', message: 'Invalid room request'}}, 400);
                }
    ...
    private async handle(request: Request): Promise<Response> {
        ...
        if (version !== null && version !== '1') {
            throw new ProtocolError('protocol_version_unsupported', 'This server speaks protocol version 1');
        }
        ...
        if (request.method === 'POST' && url.pathname === '/v1/rooms') {
            if (request.headers.get('x-can-create') !== 'true') {
                fail(401, 'Create rooms with an account token from /account');
            }
            this.chargeRequest(workspace, false);
            const raw = (await request.json()) as Record<string, unknown>;
            const input = CreateRoomRequest.parse(raw);
            const owner = request.headers.get('x-account-user');
            if (!owner) {
                fail(401, 'Account login required');
            }
            if (raw.private !== undefined && typeof raw.private !== 'boolean') {
                fail(400, 'private must be a boolean');
            }
            const allowed = await this.allowedAccounts(raw.allowedAccounts ?? []);
            const count = this.ctx.storage.sql
                .exec<{
                    n: number;
                }>("SELECT count(*) AS n FROM rooms WHERE json_extract(body,'$.lifecycle')='open' AND (json_extract(body,'$.expiresAt') IS NULL OR json_extract(body,'$.expiresAt')>?)", Date.now())
                .one().n;
            if (count >= plan.rooms) {
                fail(429, 'Concurrent room limit reached');
            }
            this.checkParticipants(input.kind);
            const policy = {
                ...DEFAULT_ROOM_POLICY,
                maxParticipants: Math.min(plan.people + plan.agents, workspace.plan === 'enterprise' ? 32 : 16),
                maxRetainedEventBytes: Math.min(plan.storageBytes, 32 * 1024 * 1024),
                maxRetainedEvents: plan.messages,
                roomLifetimeMs: null,
                inviteLifetimeMs: 86400_000
            };
            ...
            this.ctx.storage.transactionSync(() => {
                this.ctx.storage.sql.exec('INSERT INTO room_privacy VALUES(?,?,?)', created.roomId, owner, raw.private === true ? 1 : 0);
                for (const userId of new Set([owner, ...allowed])) this.ctx.storage.sql.exec('INSERT INTO room_allowed VALUES(?,?)', created.roomId, userId);
                this.ctx.storage.sql.exec('INSERT INTO room_accounts VALUES(?,?,?)', created.roomId, created.participantId, owner);
            });
        ...
        if (url.pathname === '/v1/invites/redeem' && request.method === 'POST') {
            ...
            if (!normalized) {
                fail(404, 'Invalid invite');
            }
            const invite = await this.store.inviteByDigest(await hashCredential(normalized));
            if (!invite || !this.inTeam(invite.roomId, teamId)) {
                fail(404, 'Invite not found in this team');
            }
            await this.authorizeOnlineJoin(invite.roomId, request.headers.get('x-account-user'));
            this.chargeRequest(workspace, false);
            ...
            if (!roomId || !this.inTeam(roomId, teamId)) {
                fail(404, 'Room not found in this team');
            }
            if (action === 'guests') {
                fail(403, 'Hosted rooms require an invite');
            }
            if (action === 'access') {
                fail(403, 'Hosted rooms are invite-only');
            }
            if (action === 'connect') {
                return this.connectSocket(request, roomId, active);
            }
            const credential = request.headers.get('authorization')?.replace(/^Bearer /, '');
            if (!credential) {
                fail(401, 'Room credential required');
            }
            ...
            if (action === 'allowed-accounts') {
                if (actor.kind !== 'controller') {
                    fail(403, 'Only the room controller can manage allowed accounts');
                }
                ...
                const body = (await request.json()) as {accounts?: unknown; private?: unknown};
                ...
                this.ctx.storage.transactionSync(() => {
                    this.ctx.storage.sql.exec('DELETE FROM room_allowed WHERE room_id=?', roomId);
                    for (const userId of new Set([policy.owner_id, ...accounts])) this.ctx.storage.sql.exec('INSERT INTO room_allowed VALUES(?,?)', roomId, userId);
                    this.ctx.storage.sql.exec('UPDATE room_privacy SET private=? WHERE room_id=?', body.private ? 1 : 0, roomId);
                });
                for (const socket of this.ctx.getWebSockets(roomId)) {
                    try {
                        await this.authorizeParticipant(roomId, socket.deserializeAttachment().participantId);
                    } catch {
                        socket.close(1008, 'Room access revoked');
                    }
                }
                return json({ok: true});
            }
            if (action === 'export' && request.method === 'GET') {
                return this.streamExport(roomId, toSnapshot(view), await this.store.handovers(roomId));
            }
            if (action === 'connect-ticket' && request.method === 'POST') {
                ...
                if (this.ctx.storage.sql.exec<{n: number}>('SELECT count(*) AS n FROM tickets').one().n > plan.agents + plan.people) {
                    fail(429, 'Too many connection attempts');
                }
            ...
            if (action === 'invites') {
                ...
                if (count >= (plan.agents + plan.people) * 4) {
                    fail(429, 'Invite limit reached');
                }
            }
        }
        const response = await createRouter({service: this.service, allowedOrigins: [this.env.APP_ORIGIN]})(request);
        if (response.ok && url.pathname === '/v1/invites/redeem') {
            const joined = (await response.clone().json()) as {roomId: string; participantId: string};
            const account = request.headers.get('x-account-user');
            if (account) {
                this.ctx.storage.sql.exec('INSERT OR IGNORE INTO room_accounts VALUES(?,?,?)', joined.roomId, joined.participantId, account);
            }
        }
        if (response.ok) {
            for (const event of this.emitted) await this.broadcast(event);
        }
        if (response.ok && request.method === 'DELETE' && !action && roomId) {
            for (const ws of this.ctx.getWebSockets(roomId)) ws.close(1008, 'Room deleted');
            for (const table of ['room_privacy', 'room_allowed', 'room_accounts', 'room_teams']) this.ctx.storage.sql.exec(`DELETE FROM ${table} WHERE room_id=?`, roomId);
        }
        return response;
    }
    async authorizeOnlineJoin(roomId: string, userId: string | null) {
        const view = await this.store.loadRoom(roomId);
        if (!view || view.room.lifecycle !== 'open' || (view.room.expiresAt !== null && view.room.expiresAt <= Date.now())) {
            fail(404, 'Room is unavailable');
        }
        const policy = this.ctx.storage.sql.exec<{private: number}>('SELECT private FROM room_privacy WHERE room_id=?', roomId).toArray()[0];
        if (policy?.private !== 1) {
            return;
        ...
        if (!userId) {
            fail(401, 'This private room requires an account login; run pairlobby login');
        }
        if (!this.ctx.storage.sql.exec('SELECT 1 FROM room_allowed WHERE room_id=? AND user_id=?', roomId, userId).toArray().length) {
            fail(403, 'Your account is not allowed in this private room');
        }
    }
    async checkOnlineJoin(roomId: string, userId: string | null): Promise<JoinAuthorization> {
        try {
            await this.authorizeOnlineJoin(roomId, userId);
    ...
    private async authorizeParticipant(roomId: string, participantId: string) {
        if (!this.ctx.storage.sql.exec('SELECT 1 FROM room_privacy WHERE room_id=? AND private=1', roomId).toArray().length) {
            return;
        }
        const account = this.ctx.storage.sql.exec<{user_id: string}>('SELECT user_id FROM room_accounts WHERE room_id=? AND participant_id=?', roomId, participantId).toArray()[0];
        await this.authorizeOnlineJoin(roomId, account?.user_id ?? null);
    }
    private async allowedAccounts(value: unknown): Promise<string[]> {
        if (!Array.isArray(value) || value.length > 100 || value.some((email) => typeof email !== 'string')) {
            fail(400, 'Provide at most 100 account emails');
        }
        const ids: string[] = [];
        for (const email of value as string[]) {
            const user = await this.env.DB.prepare('SELECT id FROM user WHERE lower(email)=? AND emailVerified=1').bind(email.trim().toLowerCase()).first<{id: string}>();
            if (!user) {
                fail(400, 'Every allowed email must belong to an existing verified account');
            }
            ids.push(user.id);
        }
        return ids;
    ...
    private chargeRequest(workspace: WorkspaceRecord, control: boolean) {
        ...
        if (usage.requests >= Math.floor(plan.requests * (control ? 1.2 : 1))) {
            fail(429, 'Monthly request allowance reached');
        }
    ...
    private streamExport(roomId: string, snapshot: unknown, handovers: unknown) {
        ...
        if (this.ctx.storage.sql.exec('SELECT 1 FROM exports WHERE hour=?', hour).toArray().length) {
            fail(429, 'One full export per workspace per hour is available');
        }
        ...
        const stream = new ReadableStream<Uint8Array>({
            start(controller) {
                deadline = setTimeout(() => controller.error(new Error('Export timed out; retry with a faster connection')), 60_000);
            },
            cancel() {
            ...
            async pull(controller) {
                if (!started) {
                    started = true;
                    controller.enqueue(
                        encoder.encode(JSON.stringify({room: snapshot, handovers, exportedAt: Date.now(), complete: source.earliestSeq <= 1}).slice(0, -1) + ',"events":[')
                    );
                    return;
                }
                ...
                for (const event of events) {
                    controller.enqueue(encoder.encode((first ? '' : ',') + JSON.stringify(event)));
                    first = false;
                    cursor = event.seq;
                }
                if (!page.hasMore || !events.length || cursor >= source.latestSeq) {
                    clearTimeout(deadline);
    ...
    private inTeam(roomId: string, teamId: string) {
        return this.ctx.storage.sql.exec('SELECT 1 FROM room_teams WHERE room_id=? AND team_id=?', roomId, teamId).toArray().length > 0;
    }
    private checkParticipants(kind: string) {
        const plan = PLANS[isPlan(this.current?.plan) ? this.current.plan : 'dev'];
        const row = this.ctx.storage.sql
            .exec<{n: number}>(
                `SELECT count(*) AS n FROM participants p JOIN rooms r ON r.room_id=p.room_id
            WHERE json_extract(p.body,'$.kind')=? AND json_extract(p.body,'$.leftAt') IS NULL AND json_extract(p.body,'$.revokedAt') IS NULL
            AND json_extract(r.body,'$.lifecycle')='open' AND (json_extract(r.body,'$.expiresAt') IS NULL OR json_extract(r.body,'$.expiresAt')>?)`,
                kind,
                Date.now()
            )
            .one();
        if (row.n >= (kind === 'agent' ? plan.agents : plan.people)) {
            fail(429, `${kind} session limit reached; leave an unused session first`);
        }
    }
    private meter(mutation: Mutation) {
        if (!this.current) {
            fail(402, 'No workspace is selected');
        }
        ...
        const count = this.ctx.storage.sql
            .exec<{
                ...
                receipts: number;
                replies: number;
            }>('SELECT messages,system_events,receipts,replies FROM usage WHERE period=?', this.current.period_start)
            .one();
        if (reply && count.replies >= plan.messages) {
            quota('The reserved reply allowance is exhausted');
        }
        if (receipt && count.receipts >= 2 * plan.messages + Math.max(1000, Math.floor(plan.messages * 0.05))) {
            quota('Monthly receipt allowance exhausted');
        }
        if (exempt && !receipt && !reply && count.system_events >= Math.max(1000, Math.floor(plan.messages * 0.05))) {
            quota('The reserved control and receipt allowance is exhausted');
        }
        if (!exempt && count.messages >= plan.messages) {
            quota('Monthly message allowance reached');
        }
        if (!exempt) {
            const second = Math.floor(Date.now() / 1000);
            const burst = this.ctx.storage.sql
                .exec<{events: number}>('INSERT INTO bursts(second,events) VALUES(?,1) ON CONFLICT(second) DO UPDATE SET events=events+1 RETURNING events', second)
                .one().events;
            if (burst > {dev: 5, team: 15, enterprise: 50}[isPlan(this.current.plan) ? this.current.plan : 'dev']) {
                quota('Workspace message rate exceeded; retry after one second');
            }
            this.ctx.storage.sql.exec('DELETE FROM bursts WHERE second<?', second - 1);
        }
        if (
            mutation.upsertHandovers.some((h) => !this.ctx.storage.sql.exec('SELECT 1 FROM handovers WHERE handover_id=?', h.handoverId).toArray().length) &&
            this.ctx.storage.sql.exec<{n: number}>('SELECT count(*) AS n FROM handovers WHERE room_id=?', mutation.room.roomId).one().n >= 64
        ) {
        ...
        if (!exempt) {
            this.ctx.storage.sql.exec('UPDATE usage SET messages=messages+1 WHERE period=?', this.current.period_start);
        } else if (reply) {
            this.ctx.storage.sql.exec('UPDATE usage SET replies=replies+1 WHERE period=?', this.current.period_start);
        } else if (receipt) {
            this.ctx.storage.sql.exec('UPDATE usage SET receipts=receipts+1 WHERE period=?', this.current.period_start);
        } else {
            this.ctx.storage.sql.exec('UPDATE usage SET system_events=system_events+1 WHERE period=?', this.current.period_start);
        }
    ...
    private async connectSocket(request: Request, roomId: string, active: boolean) {
        ...
        const row = this.ctx.storage.sql
            .exec<{credential: string}>('SELECT credential FROM tickets WHERE digest=? AND room_id=? AND expires>?', hash, roomId, Date.now())
            .toArray()[0];
        if (!row) {
            fail(401, 'Connection ticket invalid or expired');
        }
        ...
        if (actor.kind !== 'participant') {
            fail(403, 'Connect with a participant credential');
        }
        await this.authorizeParticipant(roomId, actor.participant.participantId);
        ...
        for (const existing of this.ctx.getWebSockets(roomId)) {
            if (existing.deserializeAttachment()?.participantId === participantId) {
    ...
    private async broadcast(event: RoomEvent) {
        ...
        for (const ws of this.ctx.getWebSockets(event.roomId)) {
            ...
            try {
                if (!view || view.room.lifecycle !== 'open' || !participant || participant.revokedAt !== null || participant.leftAt !== null) {
                    ws.close(1008, 'Room access ended');
                } else {
                    ws.send(JSON.stringify({type: 'event', event}));
                }
            } catch {
                ws.close(1011, 'Reconnect to resume');
            }
        }
    }
    override async webSocketMessage(ws: WebSocket) {
        ws.close(1008, 'Send work through the authenticated HTTP API');
    }
    ...
    override async alarm() {
        await this.serial(async () => {
            ...
            this.ctx.storage.transactionSync(() => {
                ...
                this.ctx.storage.sql.exec(
                    'DELETE FROM idempotency WHERE rowid IN(SELECT i.rowid FROM idempotency i LEFT JOIN events e ON e.room_id=i.room_id AND e.seq=i.seq WHERE e.seq IS NULL LIMIT 2000)'
                );
                this.ctx.storage.sql.exec('DELETE FROM tickets WHERE expires<=?', Date.now());
                this.ctx.storage.sql.exec('DELETE FROM reconciled_events WHERE at<?', Date.now() - 93 * 86400_000);
                this.ctx.storage.sql.exec('DELETE FROM usage_grants WHERE period<?', Date.now() - 93 * 86400_000);
                ...
                this.ctx.storage.sql.exec(
                    "DELETE FROM invites WHERE json_extract(body,'$.expiresAt') IS NOT NULL AND json_extract(body,'$.expiresAt')<? AND state='unused'",
                    Date.now()
                );
                ...
                for (const row of this.ctx.storage.sql.exec<{room_id: string; body: string}>('SELECT room_id,body FROM rooms').toArray()) {
                    ...
                    const totals = this.ctx.storage.sql
                        .exec<{
                            ...
                            b: number;
                            s: number | null;
                        }>('SELECT count(*) AS n,coalesce(sum(length(body)),0) AS b,min(seq) AS s FROM events WHERE room_id=?', row.room_id)
                        .one();
                    room.retainedEvents = totals.n;
                    room.retainedEventBytes = totals.b;
            ...
            for (const ws of this.ctx.getWebSockets()) {
                ...
                if (
                    ...
                    workspace.period_end <= Date.now() ||
                    !view ||
                    view.room.lifecycle !== 'open' ||
                    (view.room.expiresAt !== null && view.room.expiresAt <= Date.now())
                ) {
                    ws.close(1008, 'Access expired');
                }
            }
            await this.ctx.storage.setAlarm(Date.now() + 3600_000);
        });
