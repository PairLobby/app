...
export class HostedRoomStore implements RoomStore {
    ...
    constructor(private readonly storage: DurableObjectStorage, private readonly beforeApply: (mutation: Mutation) => void = () => {}) {
    ...
    private transaction<T>(run: () => T): T { return this.storage.transactionSync(run); }
    ...
    async apply(mutation: Mutation, idempotency: {key: string; requestDigest: string} | null): Promise<void> {
        this.transaction(() => {
            ...
            if (!current || JSON.parse(current.body).nextSeq !== mutation.appendEvent.seq) throw new ProtocolError('server_unavailable', 'concurrent room mutation; retry with the same idempotency key');
    ...
    async readEvents(roomId: string, after: number, limit: number): Promise<EventPage> {
        ...
        for (const row of rows.slice(0,pageLimit)) {
            ...
            events.push(JSON.parse(row.body) as RoomEvent); bytes+=size;
        }
...
class SqlDriver {
    ...
    exec(query: string) { this.sql.exec(query); }
    prepare(query: string) {
        const sql = this.sql;
        return {
            run(...values: (string | number | null)[]) { const result = sql.exec(query, ...values); return {changes: Number(sql.exec('SELECT changes() AS n').one().n)}; },
            get(...values: (string | number | null)[]) { return sql.exec(query, ...values).toArray()[0]; },
            all(...values: (string | number | null)[]) { return sql.exec(query, ...values).toArray(); },
        };
    }
}
