...
export interface WorkspaceRecord {
    id: string; name: string; owner_id: string; plan: string | null; status: string;
    customer_id: string | null; subscription_id: string | null; period_start: number; period_end: number;
}
export async function sessionUser(request: Request, env: Env, ctx: ExecutionContext): Promise<Account> {
    ...
    if (!session?.user || !session.user.emailVerified) fail(401, 'Sign in with a verified email address');
    return session.user;
}
export async function workspaceFor(env: Env, id: string, user: Account, owner = false): Promise<WorkspaceRecord> {
    ...
    if (!row || (owner && row.owner_id !== user.id)) fail(403, 'Workspace access denied');
    return row;
}
...
export async function accountRoute(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path[1] === 'accept-invite' && request.method === 'POST') {
        ...
        const invitation = await db.prepare('SELECT * FROM member_invites WHERE digest=? AND email=? AND expires_at>?').bind(hash, user.email.toLowerCase(), Date.now()).first<{workspace_id: string; team_id: string}>();
        if (!invitation) fail(404, 'Invitation is invalid, expired, or belongs to a different email');
        const workspace = await db.prepare('SELECT * FROM workspaces WHERE id=?').bind(invitation.workspace_id).first<WorkspaceRecord>();
        if (!workspace || !paid(workspace) || !isPlan(workspace.plan)) fail(402, 'An active plan is required');
        ...
        await db.batch([
            db.prepare("INSERT OR IGNORE INTO members(workspace_id,user_id,role) SELECT ?,?,'member' WHERE (SELECT COUNT(*) FROM members WHERE workspace_id=?) < ?").bind(workspace.id, user.id, workspace.id, seats),
            db.prepare('INSERT OR IGNORE INTO team_members(team_id,user_id) SELECT ?,? WHERE EXISTS(SELECT 1 FROM members WHERE workspace_id=? AND user_id=?)').bind(invitation.team_id,user.id,workspace.id,user.id),
        ]);
        const member = await db.prepare('SELECT 1 FROM members WHERE workspace_id=? AND user_id=?').bind(workspace.id,user.id).first();
        if (!member) fail(409, 'This plan has no available account seats');
        await db.prepare('DELETE FROM member_invites WHERE digest=?').bind(hash).run();
        return json({workspaceId: workspace.id});
    }
    if (path[1] !== 'workspaces') fail(404, 'Unknown account endpoint');
    ...
    const id = path[2];
    if (!id) fail(404,'Workspace not found');
    ...
    if (path.length === 3 && request.method === 'GET') {
        ...
        const [teams,members,tokens] = await Promise.all([
            db.prepare('SELECT t.* FROM teams t WHERE t.workspace_id=? AND (? OR EXISTS(SELECT 1 FROM team_members tm WHERE tm.team_id=t.id AND tm.user_id=?))').bind(id,owner ? 1 : 0,user.id).all(),
    ...
    if (path[3] === 'checkout' && request.method === 'POST') {
        const input = await body(request);
        if (!isPlan(input.plan)) fail(400,'Unknown plan');
        return json({url:await env.WORKSPACES.getByName(workspace.id).startCheckout(workspace.id,user,input.plan)});
    }
    if (path[3] === 'usage' && request.method === 'POST') return json({url:await usageCheckout(env,workspace,Number((await body(request)).blocks))});
    if (path[3] === 'portal' && request.method === 'POST') return json({url:await portal(env,workspace)});
    if (path[3] === 'members' && path[4] && request.method === 'DELETE') {
        if (path[4] === workspace.owner_id) fail(400,'The billing owner cannot be removed');
        await db.batch([
    ...
    if (!paid(workspace) || !isPlan(workspace.plan)) fail(402,'Choose an active subscription first');
    const plan = PLANS[workspace.plan];
    if (path[3] === 'teams' && request.method === 'POST') {
        const name = textField((await body(request)).name,'team name');
        const team = crypto.randomUUID();
        const inserted = await db.prepare('INSERT INTO teams(id,workspace_id,name) SELECT ?,?,? WHERE (SELECT COUNT(*) FROM teams WHERE workspace_id=?) < ?').bind(team,id,name,id,plan.teams).run();
        if (!inserted.meta.changes) fail(409,'Team limit reached');
    ...
    if (path[3] === 'invites' && request.method === 'POST') {
        ...
        if ((pending?.n ?? 0)>=50) fail(429,'Too many pending invitations');
        const token = secret();
        await db.prepare('INSERT INTO member_invites(digest,workspace_id,team_id,email,expires_at) VALUES(?,?,?,?,?)').bind(await digest(token),id,team.id,email,Date.now()+7*86400_000).run();
        return json({url:`${env.APP_ORIGIN}/account?invite=${token}`},201);
    }
    if (path[3] === 'tokens' && request.method === 'POST') {
        const input = await body(request);
        const team = await db.prepare('SELECT t.id FROM teams t WHERE t.id=? AND t.workspace_id=? AND (? OR EXISTS(SELECT 1 FROM team_members tm WHERE tm.team_id=t.id AND tm.user_id=?))').bind(textField(input.teamId,'team'),id,workspace.owner_id === user.id ? 1 : 0,user.id).first<{id:string}>();
        if (!team) fail(404,'Team not found');
        const token = `pl_${secret()}`;
        const inserted = await db.prepare('INSERT INTO api_tokens(digest,user_id,workspace_id,team_id,label,created_at,expires_at) SELECT ?,?,?,?,?,?,? WHERE (SELECT COUNT(*) FROM api_tokens WHERE user_id=? AND expires_at>?)<20').bind(await digest(token),user.id,id,team.id,textField(input.label,'token name'),Date.now(),Date.now()+90*86400_000,user.id,Date.now()).run();
        if (!inserted.meta.changes) fail(409,'Token limit reached; revoke an old token');
