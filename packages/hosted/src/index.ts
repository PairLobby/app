...
import {onlineRoute, tokenAccount} from './online';
export {Workspace} from './workspace';

export default {
    async scheduled(_event: ScheduledController, env: Env) {
        if (String(env.BILLING_ENABLED) !== 'true' || !env.STRIPE_SECRET_KEY) {
            return;
        }
        const workspaces = await env.DB.prepare(
            'SELECT id,subscription_id FROM workspaces WHERE subscription_id IS NOT NULL AND last_synced_at<? ORDER BY last_synced_at LIMIT 100'
        )
            .bind(Date.now() - 3600_000)
            .all<{id: string; subscription_id: string}>();
        for (const workspace of workspaces.results) {
            try {
                await env.WORKSPACES.getByName(workspace.id).reconcileBilling(workspace.id, workspace.subscription_id, `sync:${workspace.id}:${Math.floor(Date.now() / 900_000)}`);
            } catch {
                console.error('billing_reconciliation_failed');
            }
        }
    },
    async fetch(original: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        try {
            ...
            if (url.pathname === '/api/health') {
                return json({service: 'pairlobby', ready: true});
            }
            if (url.pathname === '/api/plans') {
                return json({currency: env.CURRENCY, plans: PLANS, signupEnabled: String(env.SIGNUPS_ENABLED) === 'true', billingEnabled: String(env.BILLING_ENABLED) === 'true'});
            }
            ...
            if (url.pathname.startsWith('/api/auth/')) {
                ...
                if (url.pathname.startsWith('/api/auth/sign-up') && String(env.SIGNUPS_ENABLED) !== 'true') {
                    fail(503, 'Signup is not open yet');
                }
                return await createAuth(env, ctx).handler(request);
            }
            if (url.pathname.startsWith('/api/online/')) {
                return await onlineRoute(request, env);
            }
            if (url.pathname.startsWith('/api/')) {
                if (request.method !== 'GET' && (!origin || origin !== env.APP_ORIGIN)) {
                    fail(403, 'Use the website to manage your account');
                }
            ...
            if (request.method === 'POST' && path === '/v1/rooms') {
                ...
                const claim = await env.DB.prepare(
                    `SELECT t.user_id FROM api_tokens t JOIN members m ON m.workspace_id=t.workspace_id AND m.user_id=t.user_id
                    ...
                    .bind(await digest(token), workspaceId, teamId, Date.now())
                    .first();
                if (!claim) {
                    fail(401, 'An account token for this team is required to create a hosted room');
                }
            ...
            if (error instanceof HttpError) {
                return json(
                    {error: {code: error.status === 429 ? 'quota_exceeded' : error.status === 401 ? 'unauthorized' : 'invalid_request', message: error.message}},
                    error.status
                );
            }
            if (error instanceof SyntaxError) {
                return json({error: {code: 'invalid_request', message: 'Invalid JSON'}}, 400);
            }
