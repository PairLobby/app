...
export function stripeClient(env: Env) {
    if (!env.STRIPE_SECRET_KEY) fail(503,'Billing is not configured yet');
...
export async function checkout(env: Env, workspace: WorkspaceRecord, user: Account, plan: Plan) {
    if (String(env.BILLING_ENABLED) !== 'true') fail(503,'Subscriptions are not open for purchase yet');
    const stripe = stripeClient(env);
    if (!prices(env)[plan]) fail(503,'This subscription is not configured');
    const price=await stripe.prices.retrieve(prices(env)[plan]);
    if (price.unit_amount!==PLANS[plan].priceCents || price.currency!==env.CURRENCY || price.recurring?.interval!=='month' || price.recurring.interval_count!==1 || !price.active) fail(503,'The billing price does not match the published plan');
    ...
    if (workspace.subscription_id) {
        ...
        if (!['canceled','incomplete_expired'].includes(existing.status)) fail(409,'Manage your existing subscription in the billing portal');
    }
    ...
    if (subscriptions.data.some(s=>!['canceled','incomplete_expired'].includes(s.status))) fail(409,'An existing subscription must be managed in the billing portal');
    ...
    const session = await stripe.checkout.sessions.create({
        customer,mode:'subscription',line_items:[{price:prices(env)[plan],quantity:1}],
        metadata:{plan},client_reference_id:workspace.id,subscription_data:{metadata:{workspaceId:workspace.id}},
        success_url:`${env.APP_ORIGIN}/account?checkout=complete`,cancel_url:`${env.APP_ORIGIN}/account`,
...
export async function portal(env: Env, workspace: WorkspaceRecord) {
    if (!workspace.customer_id) fail(400,'No billing customer exists yet');
...
export async function webhook(request: Request, env: Env) {
    if (!env.STRIPE_WEBHOOK_SECRET) fail(503,'Billing webhook is not configured');
    const signature = request.headers.get('stripe-signature');
    if (!signature) fail(400,'Missing webhook signature');
    const stripe = stripeClient(env);
    ...
    if (event.type==='checkout.session.completed' || event.type==='checkout.session.async_payment_succeeded') {
        ...
        if (checkout.metadata?.kind==='usage' && checkout.metadata.workspaceId) await env.WORKSPACES.getByName(checkout.metadata.workspaceId).reconcileUsage(checkout.metadata.workspaceId,checkout.id);
    ...
    if (!event.type.startsWith('customer.subscription.')) return json({received:true});
    ...
    if (!workspace) fail(409,'Billing customer is not yet linked; retry delivery');
...
export async function usageCheckout(env: Env, workspace: WorkspaceRecord, blocks: number) {
    if (String(env.BILLING_ENABLED)!=='true') fail(503,'Purchases are not open yet');
    if (workspace.plan!=='enterprise'||workspace.status!=='active'||workspace.period_end<=Date.now()||!workspace.customer_id) fail(402,'An active Enterprise plan is required');
    if (workspace.period_end-Date.now()<3600_000) fail(409,'Wait for the new billing period before purchasing a usage block');
    if (!Number.isInteger(blocks)||blocks<1||blocks>10) fail(400,'Choose between 1 and 10 usage blocks');
    const session=await stripeClient(env).checkout.sessions.create({
        mode:'payment',customer:workspace.customer_id,payment_method_types:['card'],expires_at:Math.floor(Date.now()/1000)+1800,
        line_items:[{price_data:{currency:env.CURRENCY,unit_amount:1000,product_data:{name:'PairLobby Enterprise usage block',description:'100,000 work events and 500,000 API calls until the current billing period ends; existing storage limits apply'}},quantity:blocks}],
