...
export function createAuth(env: Env, ctx: ExecutionContext) {
    if (!env.BETTER_AUTH_SECRET || env.BETTER_AUTH_SECRET.length<32) throw new Error('Authentication is not configured');
    const send = (to: string, subject: string, url: string) => {
        ctx.waitUntil(env.EMAIL.send({
            from: {email: 'accounts@pairlobby.com', name: 'PairLobby'}, to, subject,
        ...
        }).catch(() => { console.error('account_email_delivery_failed'); }));
    };
    return betterAuth({
        database: env.DB,
        ...
        emailAndPassword: {
            ...
            sendResetPassword: async ({user, url}) => { send(user.email, 'Reset your PairLobby password', url); },
        },
        emailVerification: {
            sendOnSignUp: true, autoSignInAfterVerification: true,
            sendVerificationEmail: async ({user, url}) => { send(user.email, 'Verify your PairLobby email', url); },
        },
