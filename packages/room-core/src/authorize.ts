...
export function assertRoomReadable(view: RoomView, now: number): void {
    if (view.room.lifecycle === 'deleted') {
        throw new ProtocolError('room_not_found', 'this room no longer exists');
    }
    if (view.room.lifecycle === 'expired' || (view.room.expiresAt !== null && now >= view.room.expiresAt)) {
        throw new ProtocolError('room_expired', 'this room has expired');
    }
...
export function assertRoomWritable(view: RoomView, now: number): void {
    ...
    if (view.room.lifecycle === 'closed') {
        throw new ProtocolError('room_closed', 'this room is closed to new activity');
    }
}

export function authenticate(view: RoomView, credentialHash: string, now: number): Actor {
    assertRoomReadable(view, now);
    if (digestsEqual(credentialHash, view.room.controllerCredentialHash)) {
    ...
    if (!participant) {
        throw new ProtocolError('unauthorized', 'credential not recognized for this room');
    }
...
export function assertActiveMember(actor: Actor): ParticipantRecord {
    ...
    if (!isActive(actor.participant)) {
        throw new ProtocolError('participant_revoked', 'this participant is no longer in the room');
    }
...
export function assertRecipientExists(view: RoomView, recipientId: string): ParticipantRecord {
    ...
    if (!recipient || !isActive(recipient)) {
