...
export function applyHandoverOffered(view: RoomView, sender: ParticipantRecord, payload: OfferPayload, recipientId: string | null, ctx: CoreContext): HandoverRecord {
    if (recipientId === null) {
        throw new ProtocolError('invalid_request', 'a handover must name its recipient');
    }
    const recipient = assertRecipientExists(view, recipientId);
    if (recipient.participantId === sender.participantId) {
        throw new ProtocolError('invalid_request', 'a handover cannot be offered to its own sender');
    }
    const existing = view.handovers.find((handover) => handover.handoverId === payload.handoverId);
    if (!existing) {
        if (payload.revision !== 1) {
            throw new ProtocolError('stale_handover_revision', 'a new handover must start at revision 1', {currentRevision: 0});
        }
        return {
    ...
    if (existing.senderId !== sender.participantId) {
        throw new ProtocolError('unauthorized', 'only the original sender may amend this handover');
    }
    if (existing.state === 'accepted') {
        throw new ProtocolError('handover_already_resolved', 'this handover was already accepted and cannot be amended');
    }
    if (payload.revision !== existing.revision + 1) {
        throw new ProtocolError('stale_handover_revision', `the next revision of this handover is ${existing.revision + 1}`, {currentRevision: existing.revision});
    }
...
function requireResolvable(view: RoomView, actor: ParticipantRecord, payload: ResolvePayload): HandoverRecord {
    ...
    if (!handover) {
        throw new ProtocolError('invalid_request', 'no such handover in this room');
    }
    if (handover.recipientId !== actor.participantId) {
        throw new ProtocolError('unauthorized', 'only the named recipient may resolve this handover');
    }
    ...
    if (handover.state !== 'offered') {
        throw new ProtocolError('handover_already_resolved', `this handover revision was already ${handover.state}; ask the sender to amend it`);
    }
    if (payload.revision !== handover.revision) {
        throw new ProtocolError('stale_handover_revision', `this handover is at revision ${handover.revision}`, {currentRevision: handover.revision});
    }
    return handover;
}
