...
export interface OpenRequest {
    eventId: string; seq: number; from: string; to: string; text: string; at: number;
    received: boolean; waitingMs: number; state?: string;
}
export function openRequests(events: RoomEvent[], now=Date.now()): OpenRequest[] {
    ...
    for(const event of ordered) {
        if(event.type==='message.received') {
            ...
            if(request?.to===event.senderId) request.received=true;
            continue;
        }
        ...
        if(event.replyTo!==null) {
            ...
            if(target && event.senderId===target.to && event.recipientId===target.from && event.payload.responseStage!=='progress') open.delete(event.replyTo);
            continue;
        }
        open.set(event.eventId,{eventId:event.eventId,seq:event.seq,from:event.senderId,to:event.recipientId,text:event.payload.text,at:event.at,received:false,waitingMs:Math.max(0,now-event.at)});
    }
    return [...open.values()];
}
