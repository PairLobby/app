...
test('migration restores every retained unanswered request without inventing receipts', async () => {
    ...
    const alice = newCredential('participant'),
    ...
    for (const text of ['first', 'second', 'third'])
        await service.send(room.roomId, alice, {type: 'message', recipientId: joined.participantId, payload: {text, priority: 'normal'}, idempotencyKey: newId('event')});
    store.close();
    const legacy = new DatabaseSync(path);
    legacy.exec("DROP TABLE message_requests; DELETE FROM meta WHERE key='message_requests_v1'");
    legacy.close();
