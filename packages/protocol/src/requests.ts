...
export function requestState(request: MessageRequest, now = Date.now()): RequestState {
    ...
    if (request.failureAt) return 'failed';
    if (request.receivedAt === null) return now - request.at >= ACK_TIMEOUT_MS ? 'ack_overdue' : 'awaiting_ack';
    return now - Math.max(request.receivedAt, request.progressAt ?? 0) >= REPLY_TIMEOUT_MS ? 'reply_overdue' : 'awaiting_reply';
}
export interface RequestPage {requests: MessageRequest[]; hasMore: boolean;}
