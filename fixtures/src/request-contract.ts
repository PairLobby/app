...
export function runRequestContract(label:string,makeStore:StoreFactory) {
    describe(`${label} / durable message obligations`,()=>{
        ...
        beforeEach(async()=>{
            store=makeStore();service=new RoomService(store);alice=newCredential('participant');bob=newCredential('participant');
            const room=await service.createRoom({name:'delivery',displayName:'alice',kind:'agent',participantCredential:alice,controllerCredential:newCredential('controller')});
            roomId=room.roomId;aliceId=room.participantId;
            bobId=(await service.redeemInvite({code:room.invite.code,displayName:'bob',kind:'agent',participantCredential:bob,attemptId:newId('attempt')})).participantId;
        });
        ...
        const answer=(id:string,text='I do not know',progress=false)=>service.send(roomId,bob,{type:'message',recipientId:aliceId,replyTo:id,payload:{text,priority:'normal',responseStage:progress?'progress':'final'},idempotencyKey:newId('event')});
        test('every request survives newer requests and unthreaded chatter',async()=>{
            const first=await ask('first'),second=await ask('second');
        ...
        test('only the recipient may acknowledge, and a final reply requires a receipt',async()=>{
            ...
            await expect(service.send(roomId,alice,{type:'message',replyTo:event.eventId,recipientId:bobId,payload:{text:'forged answer',priority:'normal'},idempotencyKey:newId('event')})).rejects.toMatchObject({code:'unauthorized'});
        ...
        test('a failed commit cannot acknowledge or close the obligation',async()=>{
            const {event}=await ask('work');
            const faulty=new FaultyStore(store);const failing=new RoomService(faulty);
            ...
            await service.acknowledgeMessage(roomId,bob,event.eventId);
            faulty.faults.failNextApply=true;
            await expect(failing.send(roomId,bob,{type:'message',recipientId:aliceId,replyTo:event.eventId,payload:{text:'answer',priority:'normal'},idempotencyKey:newId('event')})).rejects.toThrow();
        ...
        test('ordinary work quota cannot prevent a receipt and final reply',async()=>{
            const a=newCredential('participant'),b=newCredential('participant');
            const room=await service.createRoom({name:'reserved replies',displayName:'a',kind:'agent',participantCredential:a,controllerCredential:newCredential('controller'),policy:{...DEFAULT_ROOM_POLICY,maxRetainedEvents:3}});
            ...
            const sent=await service.send(room.roomId,a,{type:'message',recipientId:member.participantId,payload:{text:'last allowed request',priority:'normal'},idempotencyKey:newId('event')});
            await service.acknowledgeMessage(room.roomId,b,sent.event.eventId);
            await service.send(room.roomId,b,{type:'message',recipientId:room.participantId,replyTo:sent.event.eventId,payload:{text:'I cannot do it',priority:'normal'},idempotencyKey:newId('event')});
        ...
        test('adapter failure is visible but never forged as an agent answer',async()=>{
            ...
            expect(requestState(failed)).toBe('failed');expect(failed.receivedAt).toBeNull();expect(failed.responseEventId).toBeNull();
            expect((await service.requests(roomId,alice)).requests).toHaveLength(1);
            await service.acknowledgeMessage(roomId,bob,event.eventId);await answer(event.eventId);
